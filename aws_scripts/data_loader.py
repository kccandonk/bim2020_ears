...
def load_data(validation=False, test=True):
    ...
    if DATASET.name == "ears":
        ...
        if NETWORK.use_hog_and_landmarks:
            ...
            data_dict['X2'] = np.concatenate((data_dict['X2'], np.load(DATASET.train_folder + '/hog_features.npy')), axis=0)
        ...
        if test:
            ...
            if NETWORK.use_hog_and_landmarks:
                ...
                test_dict['X2'] = np.concatenate((test_dict['X2'], np.load(DATASET.test_folder + '/hog_features.npy')), axis=0)
