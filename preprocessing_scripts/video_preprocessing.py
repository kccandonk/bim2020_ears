...
from ears_convert_to_images_and_landmarks import *

# Paths when I did initial preprocessing attemps on AWS
...
rav_dim = [1280,720]
crop_size = 480
UPPER_BOUND = 375 # going to use 375 videos per emotion since there's only 377 max of neutral
# and this will give us a total of emotion 15,000 videos as inputs
...
if __name__=='__main__':
	...
	message1 = "Be sure to open this file and edit the local paths as necessary before running the script!"
	message2 = "From the main function, call the relevant pre-processing steps as applicable."
	print("----------------------------------------------------------------------")
	print(message1)
	print(message2)
	print("----------------------------------------------------------------------")
	...
	# # ----------------------------------------------------------------------
	# # Uncomment the function calls as needed for preprocessing
	# # ----------------------------------------------------------------------
	# # MAIN VIDEO PRE-PROCESSING SCRIPTS
	# # ----------------------------------------------------------------------
	# # Run the video pre-processing scripts
	# move_files()
	# split_videos_by_frame(UPPER_BOUND)
	# rop_all_data(UPPER_BOUND)
	# # ----------------------------------------------------------------------
	# # TRANSFORMED CROPPED FRAMES INTO IMAGES/LABELS/LANDMARKS arrays
	# # ----------------------------------------------------------------------
	# # Get the processed data
	# full_images, full_labels, full_landmarks = get_full_data()
	# # ----------------------------------------------------------------------
	# # Turn returned list into an array
	# all_images = np.asarray(full_images)
	# all_labels = np.asarray(full_labels)
	# all_landmarks = np.asarray(full_landmarks)
	# # ----------------------------------------------------------------------
	# # Save to the directory of your choosing - I did to my local and later uploaded the arrays to Google Drive
	# os.chdir('/Users/Kaitlynn/Desktop/')
	# np.save("/Users/Kaitlynn/Desktop/full_data/all_images.npy", all_images)
	# np.save("/Users/Kaitlynn/Desktop/full_data/all_labels.npy", all_labels)
	# np.save("/Users/Kaitlynn/Desktop/full_data/all_landmarks.npy", all_landmarks)
	# # ----------------------------------------------------------------------
	# # Print the shapes of the final arrays	
	# print(all_images.shape)
	# print(all_labels.shape)
	# print(all_landmarks.shape)
	# print(all_labels[0]) #check if the labels are one-hot encoded
